...
export default async function safelyHandleMessages(cChannel, pChannel, pluginName, messages) {
  ...
  try {
    processedMessageState = await sendDataToWorker(1337, [
  ...
  return Promise.all(
    processedMessageState.map(async processedMessage => {
      ...
      try {
        ...
        clog.error(`Error sending status to Lightstream Monitor for ${taskID}`);
        clog.error(e);
