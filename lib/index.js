...
export default class SpotlightPipeline {
  ...
  async start() {
    clog.info({ reason: 'STARTING_PLUGIN', message: 'Starting plugin...' });
    this.processTask();
  ...
  async _getTask() {
    clog.info({ reason: 'GET_TASK', message: 'Start polling task queue...' });
  ...
  async processTask() {
    ...
    clog.info({ reason: 'PROCESS_TASK', message: 'Processing task' });
    const {
      metadata: {
        job: {
          workflow: { type },
