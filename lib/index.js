...
export default class SpotlightPipeline {
  constructor(params) {
    ...
    this.controllerBaseURI = process.env.CONTROLLER_BASE_URI;
  ...
  async processTask() {
    ...
    const { metadata: { job: { workflow: { type } } } } = task;
  ...
  async _getTask() {
    try {
      ...
      logger.error('Unknown error getting task');
  ...
  async _handleGroup() {
    ...
    try {
      const result = await this.groupHandler(
        this.task.metadata,
        cachePath,
      );
