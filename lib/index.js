...
export default class SpotlightPipeline {
  ...
  async _getTask() {
    logger.info({ reason: 'GET_TASK', message: 'Start polling task queue...' });
  ...
  async submitResults(body) {
    logger.info({ reason: 'SUBMIT_RESULTS', message: 'Submitting results...' });
    const loop = async timeout => {
      await sleep(timeout);
      try {
        ...
        if (e.code === 'ECONNREFUSED') {
          ...
          return loop(SpotlightPipeline.exponentialTimeout(timeout));
        } else if (e.name === 'StatusCodeError' && e.StatusCode === 500) {
          /*
          Don't retry on garbage response from the controller.
          After a successful save, the plugin retries the submitting, but then the controller returns with HTTP 500.
          */
          throw e;
        } else {
          // Otherwise, just wait again
  ...
  async processTask() {
    ...
    logger.info({ reason: 'PROCESS_TASK', message: 'Processing task' });
    const {
      metadata: {
        job: {
          workflow: { type },
  ...
  async _handleGroup() {
    ...
    try {
      ...
      logger.error({ reason: 'HANDLE_GROUP', message: 'Error occurred in groupHandler function.' });
      raven.captureException(e);
