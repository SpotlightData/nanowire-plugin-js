...
export default class SpotlightPipeline {
  ...
  async processTask() {
    ...
    try {
      ...
      clog.error('Error occurred handling task: ' + e.message);
    }
  ...
  async _getTask() {
    try {
      ...
      clog.error('Error fetching task: ' + e.message);
  ...
  async _handleGroup() {
    ...
    if (cacheURL) {
      try {
        ...
        clog.error('Unknown error downloading cache...');
        raven.captureException(e);
    ...
    try {
      ...
      clog.error('Error occurred in groupHandler function.');
      raven.captureException(e);
  ...
  async _handleSingleFile() {
    ...
    try {
      ...
      clog.error('Error occurred in taskHandler function.');
      raven.captureException(e);
  ...
  async _updateResults(payload) {
    try {
      ...
      clog.error('Unknown error occurred publishing results.');
      raven.captureException(e);
